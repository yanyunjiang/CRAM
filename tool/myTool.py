...
def compute_joint_loss(ori_img, seg, seg_label, croppings, critersion, DenseEnergyLosslayer):
    ...
    return bg_celoss, fg_celoss, dloss
...
def compute_cam_up(cam, label, w, h, b):
    ...
    cam_up = cam_up * label.clone().view(b, 3, 1, 1) # torch.Size([8, 3, 368, 368])
...
def compute_dis_no_batch(seg, seg_feature):
    ...
    seg_feature_no_batch = seg_feature_no_batch.contiguous().view(-1, seg_feature.shape[1])
