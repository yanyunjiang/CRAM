...
if __name__ == '__main__':
    ...
    parser.add_argument("--infer_list", default="voc12/luad_train_list.txt", type=str)
    ...
    infer_dataset = voc12.data.VOC12ClsDatasetLUADVal(args.infer_list, voc12_root=args.voc12_root,
                scales=(1, 0.5, 1.5, 2.0),
                inter_transform=torchvision.transforms.Compose([np.asarray,
                        model.normalize,
                        imutils.HWC_to_CHW_Origin]))
    ...
    for iter, (img_name, img_list, label) in enumerate(tbar):
        ...
        if img_name != "436219-9328-58287-[1, 1, 0].png":
            continue
        else:
            ...
            # img, label = input
            # cam = model.forward_cam(img.cuda())
            ...
            # imageio.imwrite(os.path.join(args.out_cam_pred, img_name), cam)
            # quit(0)
            ...
            # new_image2 = (sum_cam*255).transpose(1,2,0).astype(np.uint8)
            # new_img2 = Image.fromarray(new_image2)
            # new_img2.save(os.path.join(args.out_cam_pred, img_name))
            # quit(0)
            ...
            # np.save("/WeaklySupervisedSemanticSegmentation/Model/model_luad/resnet38_luad_20221206_cam1step1ce/out_crf_pred25_copy13_20230226_wsss4luad_numpy/"+"{}.npy".format(img_name.split(".")[0]), norm_cam)
            # # print(norm_cam)
            # new_image1 = (norm_cam*255).transpose(1,2,0).astype(np.uint8)
            # new_img = Image.fromarray(new_image1)
            # new_img.save(os.path.join(args.out_cam_pred, img_name))
            # quit(0)
            ...
            if args.out_cam_pred is not None:
                ...
                if args.sal_path is not None:
                    sal_file = os.path.join(args.sal_path, img_name)
                ...
                imageio.imwrite(os.path.join(args.out_cam_pred, img_name.split(".")[0]+"_cam.png"), pred.astype(np.uint8))
            ...
            if args.out_crf_pred is not None:
                ...
                crf = np.zeros((4, orig_img_size[0], orig_img_size[1]), np.float32)
                ...
                if args.sal_path is not None:
                    ...
                    sal_map = np.array(Image.open(sal_file))
                    sal_map = np.where(sal_map <= args.theta * 100, 1, 0)
                    crf = np.where(crf < args.alpha, -2, crf)
                    crf = np.where(crf > args.beta, 2, crf)
                    crf[0, :, :] = sal_map
                    # crf[1, :, :] = np.multiply(crf[0, :, :], crf[1, :, :])
                    # crf[2, :, :] = np.multiply(crf[0, :, :], crf[2, :, :])
                    # crf[3, :, :] = np.multiply(crf[0, :, :], crf[3, :, :])

                pred = np.argmax(crf, 0)
                # print("p.shape:", pred.shape)
                # pred = np.multiply(1-sal_map, pred)
                imageio.imwrite(os.path.join(args.out_crf_pred, img_name.split(".")[0]+"_crfcam.png"), pred.astype(np.uint8))
